...
<head>
        ...
        <title>Alertes</title>
		<meta  http-equiv="refresh"  content="10" url="alert.php">
        <!-- Common Plugins -->
    ...
    <body>
        ...
        <section class="main-content">
            <div class="row w-no-padding margin-b-30">
				...
				<div class="row">
                    ...
                    <div class="card">
                        <div class="card-body">
                        	Les alertes
							<p class="text-muted">Liste des alertes</p>
                                    ...
                                    <tbody>
                                    	<?php 
                                    		...
                                    		foreach ($pdo->query($sql) as $row) 
											{
												...
												$id_alert=$row['id_type_alerte'];
												$heure_date = $row['heure'] .' - '.$row['date'];
												...
												foreach ($pdo->query($sql1) as $row1) {
													$num_serie=$row1['Num_serie'];
												}


												$sqla = "select * from type_alerte where id=$id_alert";
												foreach ($pdo->query($sqla) as $row2) {
													$nom_alerte=$row2['nom'];
												}

													
													echo '<td>'. $heure_date .'</td>';
													echo '<td>'.$num_serie . '</td>'; 
													echo '<td>'. $nom_alerte . '</td>'; 

												if($table_capteur == "capteur_temperature" && $nom_alerte == "Froid")
												{
												
													echo "<td class='text-center'>

											<a href='alertes/alerte_details.php?id=".$row['id']."'  ><button type='button'  class='btn btn-sm btn-primary'>
											Détails</button></a>

											<button type='button' class='btn btn-sm btn-default'>Envoyer un message</button>
												
											<button type='button' class='btn btn-sm btn-danger'>Arrêter le climatiseur</button>

											<button type='button' class='btn btn-sm btn-success'>Allumer le chauffage</button>

													</td>";
												}

												if($table_capteur == "capteur_temperature" && $nom_alerte == "Chaud")
												{
													echo "<td class='text-center'>
											...
											<button type='button' class='btn btn-sm btn-danger'>Allumer le climatiseur</button>

											<button type='button' class='btn btn-sm btn-success'>Arrêter le chauffage</button>

													</td>";
												
												}

												if($table_capteur == "capteur_detecteur_incendie" && $nom_alerte == "Feu")
												{
											...
											<button type='button' class='btn btn-sm btn-danger'>Appeller 15 (Les pompiers)</button>
            ...
            <footer class="footer">
                <span>Copyright &copy; 2019 EST Salé</span>
            </footer>
