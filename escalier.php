...
if (isset($_POST['ok'])) {
	...
	$sql = "INSERT INTO escalier (nom,description,id_etage_bas,id_etage_haut) values(?,?,?,?)";
	$q = $pdo->prepare($sql);
	$q->execute(array($nom,$description,$id_etage_bas,$id_etage_haut));
...
<html lang="fr">
    ...
    <body>
        ...
        <section class="main-content" >
     	    <div id="ajout_salle" class="row w-no-padding margin-b-30"> </div>
                    ...
                    <div class="card">
					 	...
					 	<div class="card-header card-default">
							<div class="float-right mt-12">
								<a href="javascript:%20void(0);" onclick="$('.escalier').toggle()" class="btn btn-primary btn-rounded box-shadow btn-icon"><i class="fa fa-plus"></i> Ajouter un escalier</a>
                            </div>
							Les escaliers
							<p class="text-muted">Liste des escaliers</p>
                        </div>

						<div class="card-body escalier" style="display: none;">
								...
								<div class="row">
									<div class="col-md-12">
										...
										<div class="card">
											...
											<div class="card-header card-default">
												<p class="text-muted">Entrez les informations de l'escalier que vous voullez ajouter:</p>
											</div>
							 ...
							 <div class="row">
									...
									<div class="col-md-4">
										<div class="form-group">
											...
											<select id="etage_bas" name="etage_bas" class="form-control form-control-rounded">
								...
								<div class="col-md-4">
										<div class="form-group">
											...
											<select id="etage_haut" name="etage_haut" class="form-control form-control-rounded">
											<option value="0" disabled selected hidden>Sélectionner un étage...</option>                                   
						...
						<!--------------- Liste des escaliers ------------------>
						   ...
						   <div class="card-body">
						   <table id="datatable1" class="table table-striped dt-responsive nowrap table-hover" style="text-align:center">
						   <thead>
                                        <tr>
                                            ...
                                            <th class="text-center">
                                                <strong>Etage Bas</strong>
                                            </th>
											<th class="text-center">
                                                <strong>Etage Haut</strong>
                                            </th>
                                            <th class="text-center">
									...
									<?php 
												...
												$sql = "select * from escalier";
												foreach ($pdo->query($sql) as $row) 
												{
													$id=$row['id'];
													$nom =$row['nom'];	
													$description=$row['description'];
													$id_etage_bas=$row['id_etage_bas'];
													$id_etage_haut=$row['id_etage_haut'];

													$sql1 = "select * from etage where id=$id_etage_bas";
													foreach ($pdo->query($sql1) as $row1){$nom_etage_bas =$row1['nom'];}

													$sql2 = "select * from etage where id=$id_etage_haut";
													foreach ($pdo->query($sql2) as $row2){$nom_etage_haut =$row2['nom'];}

														echo '<tr>';
														echo '<td>'. $id . '</td>';
														echo '<td>'. $nom . '</td>';
														echo '<td>'. $description . '</td>';	
														echo '<td>'. $nom_etage_bas . '</td>';
														echo "<td>". $nom_etage_haut."</td>";	

														echo "<td class='text-center'> ";

														echo "<button type='button' class='btn btn-sm btn-default' >
														<a href='escaliers/escalierPlus.php?val=".$id."'><i class='fa fa-plus'></i></a></button>";

														echo "<button type='button'    class='btn btn-sm btn-success' >
														<a href='escaliers/escalierEdit.php?val=".$id."'  ><i class='fa fa-edit'></i></a></button>";

														echo"<button type='button'    class='btn btn-sm btn-danger' >
														<a href='escaliers/escalierDelete.php?val=".$id."'  ><i class='fa fa-trash'></i></a></button>";
														echo "</td> ";
														echo "</tr> ";
												}
										?>
