...
$id_c = $_REQUEST['val'];
	...
	if (isset($_POST['modif'])) {
		...
		if($endroit == "batiment") 
		{
			$sql = "UPDATE camera set Num_serie = ?, Description = ?, Endroit = ?,Longitude = ?, Latitude = ? , adress_ip = ? ,
				  exterieur_interieur = ?,id_batiment = ? WHERE id = ?";

			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_c));
             ...
             <div class="row">
                    ...
                    <div class="card">
					 	<div class="card-header card-default">
							Camera  :
							<?php 
								$sql1 = "select * from camera where  id=$id_c";
						...
						<p class="text-muted">Modifier les informations du Camerar</p>

                        </div>
                         <div class="card-body " >
                        <?php 
                    	
							$sql = "select * from camera where  id=$id_c";
							foreach ($pdo->query($sql) as $row) 
							{
                     ...
                     <form method="post"  >
							...
							<div class="row">
								...
								<div class="col-md-4">
									<div class="form-group  ">
                                    	<label>exterieur_interieur</label>
                                    	<input type="text"  name="exterieur_interieur" value="<?php  echo $ext_int   ?>" class="form-control form-control-rounded">
                                	</div>
								</div>
