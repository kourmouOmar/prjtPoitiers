...
foreach ($pdo->query($sql) as $row){
   ...
   $password=$row['password'];
	...
	if (isset($_POST['delete'])) {
		...
		$sq = "SELECT * FROM image where id_salle=$id_salle";
		foreach ($pdo->query($sq) as $row) {$pt=$row['path'];}
		unlink("../images/$pt");

		$sql = "delete from image  WHERE id_salle= ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_etage));

		$sql = "delete from salle  WHERE id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_salle));
			...
			<br><br><br>
               <div class="container"  style="width:50%;margin:auto;background-color:white"><br>
					<div class="span10 offset1">
							...
							<h1 style="text-align:center">Supprimer une salle:</h1>
						<br>

						<form class="form-horizontal"  method="post">
							
							<p style="font-size:20pt;text-align:center">Vous êtes sur de supprimer cette salle ?</p>
							...
							<div class="form-actions" style="width:20%;margin:auto">
								...
								<a class="btn" href="../salles.php">Non</a>
							</div>
							<br>
						</form>
					</div>
