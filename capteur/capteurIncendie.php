   ...
   $password=$row['password'];
	...
	if (isset($_POST['ok'])) {
		...
		if($_POST['endroit_capteur'] == "batiment") 
		{
			...
			$sql = "INSERT INTO capteur_detecteur_incendie(Num_serie,Description,Endroit,Etat,etat_incendie,Longitude,Latitude,adress_ip,exterieur_interieur,id_batiment) values(?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id_batiment));
			header("Location: capteurIncendie.php");
		...
		if($_POST['endroit_capteur'] == "etage") 
		{
			...
			$sql = "INSERT INTO capteur_detecteur_incendie(Num_serie,Description,Endroit,Etat,etat_incendie,Longitude,Latitude,adress_ip,exterieur_interieur,id_etage) values(?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id_etage));
			header("Location: capteurIncendie.php");
		}
		...
		if($_POST['endroit_capteur'] == "salle") 
		{
			...
			$sql = "INSERT INTO capteur_detecteur_incendie(Num_serie,Description,Endroit,Etat,etat_incendie,Longitude,Latitude,adress_ip,exterieur_interieur,id_salle) values(?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id_salle));
			header("Location: capteurIncendie.php");
		}
		...
		if($_POST['endroit_capteur'] == "couloir") 
		{
			...
			$sql = "INSERT INTO capteur_detecteur_incendie(Num_serie,Description,Endroit,Etat,etat_incendie,Longitude,Latitude,adress_ip,exterieur_interieur,id_couloir) values(?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id_couloir));
			header("Location: capteurIncendie.php");
		}
		...
		if($_POST['endroit_capteur'] == "escalier") 
		{
			...
			$sql = "INSERT INTO capteur_detecteur_incendie(Num_serie,Description,Endroit,Etat,etat_incendie,Longitude,Latitude,adress_ip,exterieur_interieur,id_escalier) values(?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id_escalier));
			header("Location: capteurIncendie.php");
		}
		...
		if($_POST['endroit_capteur'] == "zone") 
		{
			...
			$sql = "INSERT INTO capteur_detecteur_incendie(Num_serie,Description,Endroit,Etat,etat_incendie,Longitude,Latitude,adress_ip,exterieur_interieur,id_zone) values(?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id_zone));
			header("Location: capteurIncendie.php");
		}
...
<head>
        ...
        <title>Détecteurs d'incendies</title>
        ...
        <section class="main-content">
					 	...
					 	<div class="card-header card-default">
							<div class="float-right mt-10">
								<a href="javascript: void(0);" class="btn btn-primary btn-rounded box-shadow btn-icon" onclick="$('.capttemp').toggle()">
									<i class="fa fa-plus"></i> Ajouter un détecteur d'incendie
								</a>
                            </div>
							Les détecteurs d'incendies
							<p class="text-muted">Liste des détecteurs d'incendies</p>
                        </div>
			...
			<div class="row">
			   ...
			   <div class="row" >
					   <div class="col-md-3">
					   <div class="form-group">
						   <label>Etat Incendie</label>
						   <input type="text" name="etat_incendie" class="form-control form-control-rounded">
					   </div>
				   </div>
				   <div class="col-md-3">
					   <div class="form-group">
                        ...
                        <div class="nv" style="padding: 15px;">
							<nav class="nav nav-pills flex-column flex-sm-row">
  								...
  								<a class="flex-sm-fill text-sm-center nav-link " href="capteurMvt.php">
   								...
   								<a class="flex-sm-fill text-sm-center nav-link active" href="capteurIncendie.php">
                    ...
                    <div class="card-body">	
                        <table id="datatable1" class="table table-striped dt-responsive nowrap table-hover">
                            ...
                            <tbody>
                                	...
                                	<?php 
                                    	$sql = "select * from  capteur_detecteur_incendie";
                                    	foreach ($pdo->query($sql) as $row) 
										{
										...
										echo '<td>'. $row['etat_incendie'] . '</td>';
										echo "
											<td class='text-center'>
											<button type='button' class='btn btn-sm btn-default'>
												<a href='capteurplus.php?id=".$row['id']."&table=capteur_detecteur_incendie' >
												<i class='fa fa-plus'></i>
												</a>
                                        	</button>
											<button type='button' class='btn btn-sm btn-success'>
												<a href='capteurIncendieEdit.php?val=".$row['id']."&tab=capteur_detecteur_incendie' >
												<i class='fa fa-edit'></i>
												</a>
											</button>
											<button type='button' class='btn btn-sm btn-danger'>
												<a href='capteurDelet.php?val=".$row['id']."&tab=capteur_detecteur_incendie'  >
