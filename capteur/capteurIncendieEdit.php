...
if($endroit == "batiment") 
{
	$sql = "UPDATE capteur_detecteur_incendie set Num_serie = ?, Description = ?, Endroit = ?, Etat = ?,
		 etat_incendie = ?,Longitude = ?, Latitude = ? , adress_ip = ? ,
		  exterieur_interieur = ?,id_batiment = ? WHERE id = ?";

	$q = $pdo->prepare($sql);
	$q->execute(array($num_serie,$description,"batiment",$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
...
if($endroit == "etage") 
{
	...
	$q = $pdo->prepare($sql);
	$q->execute(array($num_serie,$description,"etage",$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
	header("Location: capteurIncendie.php");
}
...
if($endroit == "salle") 
{
	...
	$q = $pdo->prepare($sql);
	$q->execute(array($num_serie,$description,"salle",$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
	header("Location: capteurIncendie.php");
}
...
if($endroit == "couloir") 
{
	...
	$q = $pdo->prepare($sql);
	$q->execute(array($num_serie,$description,"couloir",$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
	header("Location: capteurIncendie.php");
}
...
if($endroit == "escalier") 
{
	...
	$q = $pdo->prepare($sql);
	$q->execute(array($num_serie,$description,"escalier",$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
	header("Location: capteurIncendie.php");
}
...
if($endroit == "zone") 
{
	...
	$q = $pdo->prepare($sql);
	$q->execute(array($num_serie,$description,"zone",$etat,$etat_incendie,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
	header("Location: capteurIncendie.php");
}
}
?>
             ...
             <div class="row">
                    ...
                    <div class="card">
					 	<div class="card-header card-default">
							Capteur  :
							<?php 
								$sql1 = "select * from capteur_detecteur_incendie where  id=$id_capt";
                        ...
                        <?php 
							...
							$sql = "select * from capteur_detecteur_incendie where  id=$id_capt";
							foreach ($pdo->query($sql) as $row) 
							{
								...
								$endroit= $row['Endroit'];
								$etat= $row['Etat'];
								$etat_mouvement = $row['etat_incendie'];
	...
	<div class="row">
		<div class="col-md-4">
			<div class="form-group">
				...
				<input type="text" name="etat_incendie" value="<?php echo $etat_mouvement  ?>" class="form-control form-control-rounded">
			</div>
		</div>
