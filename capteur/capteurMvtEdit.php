...
$id_capt = $_REQUEST['val'];
	...
	if (isset($_POST['modif'])) {
		...
		if($endroit == "batiment") 
		{
			$sql = "UPDATE capteur_mouvement set Num_serie = ?, Description = ?, Endroit = ?, Etat = ?,
				 etat_mouvement = ?,Longitude = ?, Latitude = ? , adress_ip = ? ,
				  exterieur_interieur = ?,id_batiment = ? WHERE id = ?";

			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,"batiment",$etat,$etat_mouvement,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
		...
		if($endroit == "etage") 
		{
			...
			$q->execute(array($num_serie,$description,"etage",$etat,$etat_mouvement,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));			header("Location: capteurMvt.php");	
			header("Location: capteurMvt.php");
		}
		...
		if($endroit == "salle") 
		{
			...
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,"salle",$etat,$etat_mouvement,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
			header("Location: capteurMvt.php");	
		}
		...
		if($endroit == "couloir") 
		{
			...
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,"couloir",$etat,$etat_mouvement,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
			header("Location: capteurMvt.php");
		}
		...
		if($endroit == "escalier") 
		{
			...
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,"escalier",$etat,$etat_mouvement,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
			header("Location: capteurMvt.php");
		}
		...
		if($endroit == "zone") 
		{
			...
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,"zone",$etat,$etat_mouvement,$Longitude,$Latitude,$adress_ip,$ext_int,$id,$id_capt));
			header("Location: capteurMvt.php");
		}
	}  		
?>
	...
	<div class="row">
		<div class="col-md-4">
			<div class="form-group">
				...
				<input type="text" name="etat_mouvement" value="<?php echo $etat_mouvement  ?>" class="form-control form-control-rounded">
			</div>
		</div>
