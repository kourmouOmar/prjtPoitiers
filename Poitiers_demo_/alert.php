                                    ...
                                    <tbody>
                                    	<?php 
                                    		...
                                    		foreach ($pdo->query($sql) as $row) 
											{
												...
												echo '<tr>';
												echo '<td>'. $row['id'] . '</td>'; 
												echo '<td>'. $row['heure'] .' - '.$row['date'].'</td>';

												$sql1 = "select * from capteur where id_capteur=$id_capteur";
												foreach ($pdo->query($sql1) as $row) {
													$nom=$row['nom'];
												}
												echo '<td>'.$nom . '</td>'; 
												$sqla = "select * from type_alerte where id=$id_alert";
												foreach ($pdo->query($sqla) as $row) {
													$nomal=$row['nom'];
												}
												echo '<td>'. $nomal . '</td>'; 
                                    ?>
                                            <td class="text-center">
                                                <span class="label label-success" style="padding: 10px;">détail</span>
                                            </td>
