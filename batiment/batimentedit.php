...
if ($conn->connect_error) {
	...
	if (isset($_POST['modif'])) {
		...
		$sql = "UPDATE image SET path='$path' WHERE id_batiment=$id"; 
		$conn->query($sql);
		...
		if(!empty($_FILES['plan'])){
			...
			$sq = "SELECT * FROM batiment where id=$id";
			foreach ($pdo->query($sq) as $row) {$id_plann=$row['id_plan'];}
		...
		$sql = "UPDATE batiment set nom = ?, description = ? WHERE id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($nom,$description,$id));
				...
				<div class="row">
                    ...
                    <div class="card">
					 	<div class="card-header card-default">
							Les bâtiments
						<p class="text-muted">Modifier les informations du bâtiment:
                        ...
                        <?php 
							...
							$sql1 = "SELECT * FROM image where id_batiment=$id";
							foreach ($pdo->query($sql1) as $row) {$pathm=$row['path'];}
							$sql2 = "SELECT * FROM plan where id=$id_plan";
							foreach ($pdo->query($sql2) as $row) {$pathp=$row['path'];}
						?>
                     ...
                     <form method="post"  enctype="multipart/form-data" >
							<div class="row" >
								<div class="col-md-6">
									<div class="form-group  ">
                                    	<label>Nom</label>
                                    	<input type="text" name="nom" value="<?php  echo $n; ?>" class="form-control form-control-rounded">
                                	</div>
								</div>
								<div class="col-md-6">
                                   <div class="form-group">
                                    	<label>Description</label>
                                    	<input type="text" name="description" value="<?php  echo $d;  ?>" class="form-control form-control-rounded">
                                	</div>
								</div>
								<div class="col-md-6">
                                   <div class="form-group" height=300px>
										<img src="../images/<?php echo $pathm ?>" width=300px alt="<?php echo $pathm ?>">
										<input type="hidden" name="imageEN" value="<?php  echo $pathm;  ?>" >

                                	</div>
								</div>
								<div class="col-md-6">
                                   <div class="form-group "  height=300px>
										<img src="../plan/<?php echo $pathp ?>" width=300px alt="<?php echo $pathp ?>">
										<input type="hidden" name="planEN" value="<?php  echo $pathp;  ?>" >
									...
									<div class="form-group ajouter">
										<label>plan </label>
											<div class="fileinput fileinput-new input-group col-md-12" data-provides="fileinput">
											  ...
											  <span class="fileinput-new">Selectionner</span>
											  <span class="fileinput-exists">Changer</span>
											  <input type="file"  name="plan" value="" >
											  </span>
											  <a href="#" class="input-group-addon btn btn-danger  hover fileinput-exists" data-dismiss="fileinput">Supprimer</a>
											</div>
									</div>
							...
							<div class="row">
								...
								<div class="col-md-12 text-center">
									<div class="form-group btn btn-sm btn-success ">
