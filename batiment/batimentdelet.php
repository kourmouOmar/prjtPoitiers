...
<?php
		if (isset($_POST['delete'])) {
			...
			$sq = "SELECT * FROM image where id_batiment=$id";
			foreach ($pdo->query($sq) as $row) {$pt=$row['path'];}
			unlink("../images/$pt");

			$sl = "delete from image where id_batiment=?";
			$q = $pdo->prepare($sl);
			$q->execute(array($id));
			...
			$s = "SELECT * FROM image";
			foreach($pdo->query($s) as $row){	$p =$row['path'];unlink("../images/$p");}
			$s1 = "SELECT * FROM plan";
			foreach($pdo->query($s1) as $row){	$p =$row['path'];unlink("../plan/$p");}
...
<head>
        ...
        <title>Supprimer un bâtiment</title>
			...
			<br><br><br>
               <div class="container"  style="width:50%;margin:auto;background-color:white"><br>
					<div class="span10 offset1">
							...
							<h1 style="text-align:center">Supprimer un bâtiment:</h1>
						<br>

						<form class="form-horizontal"  method="post">
							
							<p style="font-size:20pt;text-align:center">Vous êtes sur de supprimer ce bâtiment ?</p>
							<input type="hidden" name="id" value="<?php echo $id;?>"/><br>
							<div class="form-actions" style="width:25%;margin:auto">
								<button type="submit" name="delete" class="btn btn-danger">Oui</button>
								<a class="btn" href="../batiment.php">Non</a>
							</div>
							<br>
						</form>
					</div>
