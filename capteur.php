   ...
   $password=$row['password'];
	...
	if (isset($_POST['ok'])) {
		...
		if($_POST['endroit_capteur'] == "batiment") 
		{
			...
			$sql = "INSERT INTO capteur_temperature(Num_serie,Description,Endroit,Etat,Temp_max,Temp_min,Temp_reelle,Longitude,Latitude,adress_ip,exterieur_interieur,id_batiment) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$tempmax,$tempmin,$tempreel,$Longitude,$Latitude,$adress_ip,$ext_int,$id_batiment));
		...
		if($_POST['endroit_capteur'] == "etage") 
		{
			...
			$sql = "INSERT INTO capteur_temperature(Num_serie,Description,Endroit,Etat,Temp_max,Temp_min,Temp_reelle,Longitude,Latitude,adress_ip,exterieur_interieur,id_etage) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$tempmax,$tempmin,$tempreel,$Longitude,$Latitude,$adress_ip,$ext_int,$id_etage));
			header("Location: capteur.php");
		}
		...
		if($_POST['endroit_capteur'] == "salle") 
		{
			...
			$sql = "INSERT INTO capteur_temperature(Num_serie,Description,Endroit,Etat,Temp_max,Temp_min,Temp_reelle,Longitude,Latitude,adress_ip,exterieur_interieur,id_salle) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$tempmax,$tempmin,$tempreel,$Longitude,$Latitude,$adress_ip,$ext_int,$id_salle));
			header("Location: capteur.php");
		}
		...
		if($_POST['endroit_capteur'] == "couloir") 
		{
			...
			$sql = "INSERT INTO capteur_temperature(Num_serie,Description,Endroit,Etat,Temp_max,Temp_min,Temp_reelle,Longitude,Latitude,adress_ip,exterieur_interieur,id_couloir) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$tempmax,$tempmin,$tempreel,$Longitude,$Latitude,$adress_ip,$ext_int,$id_couloir));
			header("Location: capteur.php");
		}
		...
		if($_POST['endroit_capteur'] == "escalier") 
		{
			...
			$sql = "INSERT INTO capteur_temperature(Num_serie,Description,Endroit,Etat,Temp_max,Temp_min,Temp_reelle,Longitude,Latitude,adress_ip,exterieur_interieur,id_escalier) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$tempmax,$tempmin,$tempreel,$Longitude,$Latitude,$adress_ip,$ext_int,$id_escalier));
			header("Location: capteur.php");
		}
		...
		if($_POST['endroit_capteur'] == "zone") 
		{
			...
			$sql = "INSERT INTO capteur_temperature(Num_serie,Description,Endroit,Etat,Temp_max,Temp_min,Temp_reelle,Longitude,Latitude,adress_ip,exterieur_interieur,id_zone) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			$q = $pdo->prepare($sql);
			$q->execute(array($num_serie,$description,$endroit,$etat,$tempmax,$tempmin,$tempreel,$Longitude,$Latitude,$adress_ip,$ext_int,$id_zone));
			header("Location: capteur.php");
		}
					 	...
					 	<div class="card-header card-default">
							<div class="float-right mt-10">
								<a href="javascript: void(0);"  class="btn btn-primary btn-rounded box-shadow btn-icon" onclick="$('.capttemp').toggle()">
									<i class="fa fa-plus"></i> Ajouter un capteur de températeure
								</a>
                            </div>
							Les capteurs de température
							<p class="text-muted">Liste des capteurs de température</p>
                        </div>
                        ...
                        <div class="card-body capttemp" style="display: none;">
						  ...
						  <div class="nv" style="padding: 15px;">
							<nav class="nav nav-pills flex-column flex-sm-row">
  								<a class="flex-sm-fill text-sm-center nav-link active" href="capteur.php">
  									Capteur du température
  								</a>
  								<a class="flex-sm-fill text-sm-center nav-link" href="capteur/capteurHumidite.php">
  									Capteur d'humidité
  								</a>
  								<a class="flex-sm-fill text-sm-center nav-link" href="capteur/capteurMvt.php">
  									Capteur de mouvement
  								</a>
   								<a class="flex-sm-fill text-sm-center nav-link" href="capteur/capteurIncendie.php">
                        ...
                        <table id="datatable1" class="table table-striped dt-responsive nowrap table-hover">
                            <thead>
                            	<tr>
                                    ...
                                    <th class="text-center">
                                        <strong>Endroid</strong>
                                    </th>
                                    <th class="text-center">
                                        <strong>Etat</strong>
                                    </th>
                                    <th class="text-center">
                                        <strong>Température réelle</strong>
                                    </th>
                                    <th class="text-center">
                            ...
                            <tbody>
                                	...
                                	<?php 
                                    	$sql = "select * from  capteur_temperature order by endroit";
                                    	foreach ($pdo->query($sql) as $row) 
										{
										...
										echo '<td>'. $row['Etat'] . '</td>';
										echo '<td>'. $row['Temp_reelle'] . '</td>';
										
										echo "
											<td class='text-center'>
                                        	<button type='button' class='btn btn-sm btn-default'>
												<a href='capteur/capteurplus.php?id=".$row['id'].
												"&table=capteur_temperature  >
                                        		<i class='fa fa-plus'></i>
                                        		</a>
                                        	</button>
											<button type='button' class='btn btn-sm btn-success'>
												<a href='capteur/capteurtemperateurEdit.php?val=".$row['id']."&tab=capteur_temperature'>
												<i class='fa fa-edit'></i>
												</a>
											</button>
											<button type='button' class='btn btn-sm btn-danger'>
												<a href='capteur/capteurDelet.php?val=".$row['id']."&tab=capteur_temperature'>
