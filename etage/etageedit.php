...
$id_etage = $_REQUEST['val'];
	...
	if (isset($_POST['modif'])) {
		...
		if(!empty($_FILES['image'])){
			...
			$sql = "UPDATE image SET path='$path' WHERE id_etage=$id_etage"; 
			$conn->query($sql);
		...
		if(!empty($_FILES['plan'])){
			...
			foreach ($pdo->query($sq) as $row) {
				$id_plann=$row['id_plan'];
			}

			$sql = "UPDATE plan SET path='$path' WHERE id=$id_plann"; 
			$conn->query($sql);
			 
			unlink("../plan/$planEN");
		}
		
		$sql = "UPDATE etage set nom = ?, description = ? WHERE id = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($nom,$description,$id_etage));
             ...
             <div class="row">
                    ...
                    <div class="card">
					 	<div class="card-header card-default">
							Modification de l'étage
						<p class="text-muted">Modifier les informations que vous voulez changer:</p>
							...
							<div class="col-md-6">
								<div class="form-group  ">
									<label>Nom du batiment</label>
										<input type="text" name="nom_batiment" disabled value="<?php  echo $nom_batiment ?>" class="form-control form-control-rounded">
											</div>
									</div>
									<div class="col-md-6">
									<div class="form-group">
										<label>Description du batiment</label>
										<input type="text" name="description_batiment" disabled value="<?php  echo $des_batiment  ?>" class="form-control form-control-rounded">
									</div>
								</div>
							</div>
							<div class="row">
								<div class="col-md-6">
									<div class="form-group  ">
                                    	...
                                    	<input type="text" name="nom" value="<?php  echo $nom_etage ?>" class="form-control form-control-rounded">
                                	</div>
								</div>
								<div class="col-md-6">
                                   <div class="form-group">
                                    	<label>Description de l'étage</label>
                                    	<input type="text" name="description" value="<?php  echo $des_etage  ?>" class="form-control form-control-rounded">
                                	</div>
								</div>
								<div class="col-md-6">
                                   <div class="form-group">
                                    	<label>plan</label><br>
                                    	<?php 
											$sql1 = "select * from plan where  id=$id_pln";
											foreach ($pdo->query($sql1) as $row) {$pathp= $row['path'] ;}
										
										?>
										<img src="../plan/<?php echo $pathp; ?>" alt="" width="300px">
										<input type="text" name="planEN" value="<?php  echo $pathp;  ?>" >
                                	</div>
								</div>
								<div class="col-md-6">
                                   <div class="form-group">
                                    	...
                                    	<?php 
											$sql1 = "select * from image where  id_etage=$id_etage";
											foreach ($pdo->query($sql1) as $row) {$pathm= $row['path'] ;}
										
										?>
										<input type="text" name="imageEN" value="<?php  echo $pathm;  ?>" >

										<img src="../images/<?php echo $pathm; ?>" alt="" width="300px">
                                	</div>
								</div>
							</div>
